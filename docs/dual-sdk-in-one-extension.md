...
无需改。照常「需要时 `createDocument`(Reclaim 的 offscreen URL)，用完关闭」。
...
- **文件 1**：`trex-proxy-browser-extension-sdk/src/utils/offscreen-manager.js`
...
- **文件 2**：`trex-proxy-browser-extension-sdk/src/utils/proof-generator/proof-generator.js`
...
- **可选**：若希望「整轮验证会话只关一次」，可在 `trex-proxy-browser-extension-sdk/src/background/proofQueue.js` 的 `processNextQueueItem` 的 `finally` 中，当 `ctx.proofGenerationQueue.length === 0` 且即将调用 `ctx.submitProofs()` 时，再调用 `closeOffscreenDocument()`；并去掉 proof-generator 里每次成功就关的逻辑，避免同一会话内多次开关。

**步骤 4：宿主扩展（可选）**

- 在宿主扩展的 background（如 `trex-extension/src/pages/background/index.ts`）中：
...
1. 改 `trex-proxy-browser-extension-sdk/src/utils/offscreen-manager.js` 的 offscreen URL 为 `trex-browser-extension-sdk/...`（步骤 1）。  
2. 在同一文件 `ensureOffscreenDocument` 中加入「已有 OFFSCREEN_DOCUMENT 时轮询等待直至消失再创建」及超时（步骤 2）。  
3. 在 `trex-proxy-browser-extension-sdk/src/utils/offscreen-manager.js` 新增并导出 `closeOffscreenDocument`，在 `trex-proxy-browser-extension-sdk/src/utils/proof-generator/proof-generator.js` 的 proof 成功回调里调用它（步骤 3）。  
4. 按需在宿主扩展（如 `trex-extension/src/pages/background/index.ts`）做 offscreen 使用权标记（步骤 4）。
